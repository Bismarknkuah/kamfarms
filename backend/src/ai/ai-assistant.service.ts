...
export class AiAssistantService {
  ...
  async ask(dto: AskAssistantDto): Promise<AssistantAnswer> {
    ...
    if (q.includes('highest output') || (q.includes('farm') && q.includes('output'))) {
      // farmReport now requires an actor for scoping (a real gap fixed
      // this batch - it used to accept any farmId with no check at
      // all). This specific query is inherently a cross-farm
      // comparison, so a synthetic GLOBAL actor preserves the existing
      // company-wide answer rather than half-fixing this into a
      // narrower, less useful response - the AI assistant module has
      // no real per-caller actor threading at all yet, which is a
      // separate, pre-existing gap beyond this fix's scope.
      const globalActor = { id: 'system', roles: [{ scopes: [{ scopeType: 'GLOBAL', scopeId: null }] }] } as unknown as AuthenticatedUser;
      const farms = await this.reports.farmReport({}, globalActor);
