...
export class AuthService {
  ...
  async requestPasswordReset(email: string) {
    ...
    // Real SMTP delivery, wired through EmailService - the fix behind
    // this method's own previous scope note ("not wired to an actual
    // provider yet"). EmailService itself still falls back to a
    // visible warn-level log when EMAIL_HOST isn't configured, so an
    // unconfigured deployment degrades to exactly the old, usable-for-
    // demo behavior rather than silently losing the token.
