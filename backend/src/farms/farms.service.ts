...
export class FarmsService {
  ...
  async getInventory(farmId: string) {
    ...
    // "what have been sent" - every real dispatch order ever created
    // against this farm, by size, regardless of its current status.
    // Deliberately every order (not just completed ones): a Farm
    // Manager asking "what have I sent" means everything they've
    // committed to dispatching, not only shipments that have already
    // arrived and been reconciled.
    const dispatchOrders = await this.prisma.deliveryOrder.findMany({
      where: { farmId },
      include: { paddyGrade: true },
    });
    const dispatchedByGradeMap = new Map<string, { gradeCode: string; gradeLabel: string; bagCount: number; totalKg: number }>();
    for (const order of dispatchOrders) {
      const existing = dispatchedByGradeMap.get(order.paddyGradeId) ?? {
        gradeCode: order.paddyGrade.code,
        gradeLabel: order.paddyGrade.label,
        bagCount: 0,
        totalKg: 0,
      };
      existing.bagCount += order.bagCount;
      existing.totalKg += Number(order.totalKg);
      dispatchedByGradeMap.set(order.paddyGradeId, existing);
    }
    const dispatchedByGrade = Array.from(dispatchedByGradeMap.values());
    const dispatchedTotalKg = dispatchedByGrade.reduce((sum, g) => sum + g.totalKg, 0);
    const dispatchedTotalBags = dispatchedByGrade.reduce((sum, g) => sum + g.bagCount, 0);

    return { farmId, byGrade, totalKg, totalBags, dispatchedByGrade, dispatchedTotalKg, dispatchedTotalBags };
  }
}
