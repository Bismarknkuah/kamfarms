...
describe('DeliveryReportsService.update - editable up until approval, not just while a draft', () => {
  const farmManagerA = { id: 'manager-1', roles: [{ scopes: [{ scopeType: 'FARM', scopeId: 'farm-a' }] }] } as unknown as AuthenticatedUser;
  ...
  const submittedReport = {
    ...
    otherCosts: 0,
    farm: {}, destinationWarehouse: {}, paddyGrade: {}, vehicle: null, driver: null,
    submittedBy: {}, approvedBy: null, shipment: null, deliveryOrder: {},
  };

  function buildUpdateService(report = submittedReport) {
    const prisma = {
      deliveryReport: {
      ...
      $transaction: jest.fn((cb: (tx: unknown) => unknown) =>
        cb({
          deliveryReport: { update: jest.fn().mockResolvedValue({ ...report, actualBagCount: 410 }) },
        }),
      ),
    };
    const audit = { record: jest.fn() } as unknown as AuditService;
  ...
  it('allows the original submitter to edit a report that is already SUBMITTED, not just DRAFT - the real gap this fixes', async () => {
    const { service } = buildUpdateService();
    await expect(service.update('dr-1', { actualBagCount: 410 }, farmManagerA)).resolves.toBeDefined();
  });

  it('refuses to edit a report that has already been APPROVED', async () => {
    const { service } = buildUpdateService({ ...submittedReport, status: 'APPROVED' });
    await expect(service.update('dr-1', { actualBagCount: 410 }, farmManagerA)).rejects.toThrow(BadRequestException);
  });

  it('refuses to let anyone other than the original submitter edit the report', async () => {
    const { service } = buildUpdateService();
    const someoneElse = { id: 'manager-2', roles: [{ scopes: [{ scopeType: 'FARM', scopeId: 'farm-a' }] }] } as unknown as AuthenticatedUser;
    await expect(service.update('dr-1', { actualBagCount: 410 }, someoneElse)).rejects.toThrow(ForbiddenException);
  });
});
