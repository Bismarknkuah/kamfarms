...
describe('PaddyEntriesService', () => {
  ...
  describe('update() - editable up until approval, not just while still a draft', () => {
    it('allows the original submitter to edit an entry that is already SUBMITTED, not just DRAFT', async () => {
      const prisma = {
        paddyEntry: {
          findUnique: jest.fn().mockResolvedValue(submittedEntry), // status: 'SUBMITTED'
        },
        $transaction: jest.fn((cb: (tx: unknown) => unknown) =>
          cb({ paddyEntry: { update: jest.fn().mockResolvedValue({ ...submittedEntry, bagCount: 1100 }) } }),
        ),
      };
      const audit = { record: jest.fn() } as unknown as AuditService;
      ...
      await expect(service.update('pe-1', { bagCount: 1100 }, actor)).resolves.toBeDefined();
    });

    it('refuses to edit an entry that has already been APPROVED', async () => {
      const prisma = {
        paddyEntry: { findUnique: jest.fn().mockResolvedValue({ ...submittedEntry, status: 'APPROVED' }) },
        $transaction: jest.fn(),
      };
      const audit = { record: jest.fn() } as unknown as AuditService;
      const ledger = {} as unknown as InventoryLedgerService;
      ...
      await expect(service.update('pe-1', { bagCount: 1100 }, actor)).rejects.toThrow(BadRequestException);
    });
  });
});
