...
export class PaddyEntriesController {
  ...
  @Get(':id/comments')
  @RequirePermission([PERMISSIONS.PADDY_CREATE, PERMISSIONS.PADDY_APPROVE, PERMISSIONS.PADDY_REJECT])
  listComments(@Param('id') id: string, @CurrentUser() actor: AuthenticatedUser) {
    return this.paddyEntriesService.listComments(id, actor);
  }

  @Post(':id/comments')
  @RequirePermission([PERMISSIONS.PADDY_CREATE, PERMISSIONS.PADDY_APPROVE, PERMISSIONS.PADDY_REJECT])
  addComment(@Param('id') id: string, @Body() dto: CreatePaddyEntryCommentDto, @CurrentUser() actor: AuthenticatedUser) {
    return this.paddyEntriesService.addComment(id, dto, actor);
  }
}
