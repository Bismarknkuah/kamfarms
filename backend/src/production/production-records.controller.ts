...
export class ProductionRecordsController {
  ...
  @Get('predict')
  @RequirePermission(PERMISSIONS.MILLING_VIEW)
  predict(@Query('paddyGradeId') paddyGradeId: string, @Query('bagCount') bagCount: string) {
    return this.productionRecordsService.predictYield(paddyGradeId, parseInt(bagCount, 10));
  }
