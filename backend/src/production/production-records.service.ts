...
export class ProductionRecordsService {
    ...
    const record = await this.prisma.$transaction(async (tx) => {
      ...
      const created = await tx.productionRecord.create({
        data: {
          ...
          paddyProcessedBags: dto.paddyProcessedBags,
          sourceReferenceNumbers: dto.sourceReferenceNumbers ?? [],
  ...
  /** A real, honest prediction, not a fabricated one: averages actual
   * historical recovery/broken/hull percentages and energy-per-KG from
   * this grade's own approved production records, then projects those
   * same ratios onto the bag count someone's about to send. Returns
   * null (not a guessed number) when there's no history for this grade
   * yet - an honest "not enough data" beats a confident-looking number
   * with nothing real behind it. This is what lets a Warehouse
   * Supervisor know "send 40 bags of Size 4, expect roughly this much
   * rice, this much broken rice, this much hull, and this much power
   * used" before it happens, not just see it after the fact. */
  async predictYield(paddyGradeId: string, bagCount: number) {
    const STANDARD_PADDY_BAG_WEIGHT_KG = 50;
    const inputKg = bagCount * STANDARD_PADDY_BAG_WEIGHT_KG;

    const history = await this.prisma.productionRecord.findMany({
      where: { paddyGradeId, status: 'APPROVED' },
      select: {
        paddyProcessedKg: true,
    ...
    if (history.length === 0) {
      return { hasHistory: false, sampleSize: 0 };
    }

    const avg = (values: number[]) => values.reduce((sum, v) => sum + v, 0) / values.length;
    const avgRecoveryPercent = avg(history.map((r) => Number(r.recoveryPercent)));
    const avgBrokenPercent = avg(history.map((r) => Number(r.brokenPercent)));
    const avgHullPercent = avg(history.map((r) => Number(r.hullPercent)));
    const avgWastePercent = avg(history.map((r) => Number(r.wastePercent)));

    // Energy prediction only uses records that actually logged a meter
    // reading - most historical records may not have, and averaging in
    // zeros would silently understate the real per-KG cost.
    const withEnergy = history.filter((r) => r.energyConsumptionKwh !== null && Number(r.paddyProcessedKg) > 0);
    const avgKwhPerKg = withEnergy.length > 0
      ? avg(withEnergy.map((r) => Number(r.energyConsumptionKwh) / Number(r.paddyProcessedKg)))
      : null;
    ...
    return {
      ...
      inputKg,
      expectedRecoveredKg: inputKg * (avgRecoveryPercent / 100),
      expectedBrokenKg: inputKg * (avgBrokenPercent / 100),
      expectedHullKg: inputKg * (avgHullPercent / 100),
      expectedWasteKg: inputKg * (avgWastePercent / 100),
      expectedEnergyKwh: avgKwhPerKg !== null ? avgKwhPerKg * inputKg : null,
      basedOnRecoveryPercent: avgRecoveryPercent,
      basedOnBrokenPercent: avgBrokenPercent,
      basedOnHullPercent: avgHullPercent,
    };
  }
}
