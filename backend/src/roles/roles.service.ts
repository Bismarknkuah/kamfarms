...
import { UpdateRoleDetailsDto } from './dto/update-role-details.dto';
...
export class RolesService {
  ...
  async updateDetails(code: string, dto: UpdateRoleDetailsDto, actor: AuthenticatedUser) {
    const role = await this.findByCode(code);
    // Deliberately only name/description are editable here, never
    // `code` - the role code is a hardcoded identifier checked
    // throughout the frontend (roleCodes.includes('FARM_MANAGER') and
    // similar), and every built-in nav/dashboard section keyed to a
    // specific role would silently break if that string ever changed
    // underneath it. "Edit the name" means the human-readable label,
    ...
    await this.audit.record({
      userId: actor.id,
      action: 'role.update_details',
      entity: 'Role',
      entityId: role.id,
      beforeValue: { name: role.name, description: role.description },
      afterValue: { name: updated.name, description: updated.description },
    });
