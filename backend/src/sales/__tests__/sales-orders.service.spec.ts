...
describe('SalesOrdersService', () => {
  ...
  function buildService(overrides: Partial<{ balanceBagCount: number; activeReservedBags: number }> = {}) {
    // Mutable, not a static return value - a real database reflects
    // whatever the transaction just wrote, and findById() (called at
    // the end of approve()) re-reads through the top-level client, not
    // the transaction's own tx object. A static mock here would always
    // hand back the pre-approval state regardless of what the
    // transaction did, silently making success and failure paths
    // indistinguishable to the test.
    let currentOrder: Record<string, unknown> = { ...submittedOrder };
    const prisma = {
      ...
      $transaction: jest.fn((cb: (tx: unknown) => unknown) =>
        cb({
          ...
          task: { count: jest.fn().mockResolvedValue(0), create: jest.fn() },
        }),
      ),
    };
