...
export class WarehousesService {
  ...
  /** Name-only, active warehouses - for picking a dispatch destination,
   * never for browsing warehouse data. Deliberately excludes stock,
   * managers, milling centers - everything the fuller warehouse.view
   * grant exposes - since a Farm Manager needs this list without ever
   * gaining visibility into warehouse inventory. */
  directory() {
    return this.prisma.warehouse.findMany({
      where: { isActive: true },
