...
export default function DeliveriesPage() {
  ...
  const onCreateOrder = async () => {
    if (!accessToken || !orderFarmId || !orderWarehouseId || validOrderRows.length === 0) return;
    setCreatingOrder(true);
    setPageError(null);
    try {
      // Same pattern as every other multi-size form this session -
      // each size becomes its own real order, since DeliveryOrder is
      // single-grade at the schema level.
      for (const row of validOrderRows) {
        await deliveryOrdersApi.create(accessToken, {
          ...
          paddyGradeId: row.paddyGradeId,
          bagCount: parseInt(row.bagCount, 10),
          totalKg: row.totalKg ? parseFloat(row.totalKg) : undefined,
        });
      }
    ...
    <DashboardShell me={me}>
      ...
      {tab === 'Dispatch orders' && (
        <div className="mt-6">
          ...
          {showCreateOrder && (
            <div className="mb-4 rounded-2xl border border-husk-300 bg-husk-100/30 p-4">
              ...
              <div className="mt-4 flex items-center justify-between">
                <p className="text-xs font-semibold uppercase tracking-wide text-soil-500">Sizes in this dispatch</p>
                <button
                  type="button"
                  onClick={() => setOrderRows(grades.map((g) => ({ paddyGradeId: g.id, bagCount: '', totalKg: '' })))}
                  className="text-xs font-medium text-paddy-700 underline"
                >
                  Select all sizes
                </button>
              </div>
              <p className="mt-0.5 text-xs text-ink-500">Add one row per size, or use &ldquo;Select all sizes&rdquo; and fill in bags for each.</p>
                ...
                {orderRows.map((row, index) => {
                  ...
                  return (
                    <div key={index} className="grid gap-3 sm:grid-cols-[1fr_1fr_1fr_auto]">
                      ...
                      <div>
                        {index === 0 && <label className="mb-1 block text-xs font-medium text-ink-700">Total bags</label>}
                        <input type="number" value={row.bagCount} onChange={(e) => updateOrderRow(index, 'bagCount', e.target.value)} placeholder="Required" className="w-full rounded-lg border border-paddy-100 px-2 py-1.5 text-sm" />
                      </div>
                      <div>
                        {index === 0 && <label className="mb-1 block text-xs font-medium text-ink-700">Weight (KG)</label>}
                        <input type="number" value={row.totalKg} onChange={(e) => updateOrderRow(index, 'totalKg', e.target.value)} placeholder="Optional" className="w-full rounded-lg border border-paddy-100 px-2 py-1.5 text-sm" />
                      </div>
                      {orderRows.length > 1 && (
                        <div className={index === 0 ? 'mt-5' : ''}>
                          <button type="button" onClick={() => removeOrderRow(index)} className="rounded-lg border border-red-200 px-3 py-1.5 text-sm text-red-600 hover:bg-red-50">
                            Remove
                          </button>
                        </div>
                      )}
              ...
              <button type="button" onClick={onCreateOrder} disabled={creatingOrder || !orderFarmId || !orderWarehouseId || validOrderRows.length === 0} className="mt-4 rounded-full bg-paddy-900 px-5 py-2 text-sm font-medium text-rice-50 disabled:opacity-50">
                {creatingOrder ? 'Creating…' : `Create ${validOrderRows.length > 1 ? `${validOrderRows.length} orders` : 'order'}`}
              </button>
            </div>
          )}
