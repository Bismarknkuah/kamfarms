...
import {
  farmsApi, paddyEntriesApi, deliveryOrdersApi, farmEquipmentApi,
  Farm, PaddyEntry, DeliveryOrder, FarmEquipment,
  ApiError,
} from '@/lib/api-client';
...
export default function FarmDetailPage() {
  ...
  useEffect(() => {
    ...
    farmEquipmentApi.list(accessToken, farmId).then(setEquipment).catch(() => {});
              ...
              <div className="mt-3 grid gap-2 sm:grid-cols-2">
                {equipment.map((eq) => (
                  <div key={eq.id} className="flex items-center justify-between rounded-lg bg-rice-50 px-3 py-2 text-sm">
                    <div>
                      <span className="text-ink-900">{eq.name}</span>
