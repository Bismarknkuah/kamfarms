      ...
      {farms && (
        <div className="mt-6 grid gap-4 lg:grid-cols-2">
          {farms.map((farm) => (
            <div
              key={farm.id}
              className={`overflow-hidden rounded-2xl border bg-white ${farm.isActive ? 'border-paddy-100' : 'border-ink-500/20 opacity-70'}`}
            >
              <div className={`flex items-center gap-3 border-b px-5 py-4 ${farm.isActive ? 'border-paddy-100 bg-paddy-900' : 'border-ink-500/20 bg-ink-500/5'}`}>
                <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${farm.isActive ? 'bg-husk-500/20' : 'bg-ink-500/10'}`}>
                  <Sprout className={`h-5 w-5 ${farm.isActive ? 'text-husk-300' : 'text-ink-500'}`} />
                </div>
                <div className="min-w-0 flex-1">
                  <p className={`font-mono text-xs ${farm.isActive ? 'text-paddy-300' : 'text-ink-500'}`}>{farm.code}</p>
                  {editingFarmId !== farm.id && (
                    <Link href={`/farms/${farm.id}`} className={`block truncate font-display text-lg hover:underline ${farm.isActive ? 'text-rice-50' : 'text-ink-700'}`}>
                      {farm.name}
                    </Link>
                  )}
                </div>
                {!farm.isActive && <span className="shrink-0 rounded-full bg-ink-500/10 px-2 py-0.5 text-xs font-medium text-ink-500">Inactive</span>}
              </div>
              ...
              <div className="p-5">
                {editingFarmId === farm.id ? (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-1.5 text-sm text-ink-500">
                      ...
                      <span>{farm.location ?? 'No location set'}</span>
                    </div>
                    {canManage && (
                ...
                <Link href={`/farms/${farm.id}`} className="mt-2 inline-block text-xs font-medium text-husk-700 hover:underline">
                  View details →
                </Link>
