...
export default function TasksPage() {
  ...
  const onComplete = async (taskId: string) => {
    ...
    try {
      await tasksApi.updateStatus(accessToken, taskId, 'COMPLETED', evidence, attachmentUrl.trim() || undefined);
        ...
        <div className="mt-6 space-y-3">
          {tasks.map((task) => (
            <div key={task.id} className="rounded-2xl border border-paddy-100 bg-white p-5">
              ...
              {task.status === 'COMPLETED' && (task.completionEvidence || task.attachmentUrl) && (
                <div className="mt-4 border-t border-paddy-100 pt-4 text-sm">
                  {task.completionEvidence && <p className="text-ink-700">{task.completionEvidence}</p>}
                  {task.attachmentUrl && (
                    <a href={task.attachmentUrl} target="_blank" rel="noopener noreferrer" className="mt-1 inline-block text-paddy-700 underline">
                  ...
                  {completingId === task.id ? (
                    <div className="space-y-2">
                      ...
                      <input
                        ...
                        placeholder="Link to a report or document, if one applies (optional) - Drive, Dropbox, etc."
                        className="w-full rounded-lg border border-paddy-100 px-3 py-2 text-sm outline-none focus:border-paddy-500 focus:ring-2 focus:ring-paddy-500/20"
                      />
