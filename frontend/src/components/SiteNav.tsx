...
// scrolls into the content - the navbar behaves like a cover page
// that becomes a header, rather than a bar that sits on top of the
  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-40 transition-all duration-500 ${
        solid ? 'border-b border-paddy-100 bg-rice-50/95 py-3 shadow-sm backdrop-blur' : 'bg-transparent py-6'
      }`}
    >
      <div className="mx-auto flex max-w-6xl items-center justify-between px-6">
        ...
        <Link
          ...
          className={`rounded-full px-5 py-2 text-sm font-medium transition ${
            solid ? 'bg-paddy-900 text-rice-50 hover:bg-paddy-700' : 'border border-rice-50/60 text-rice-50 hover:bg-rice-50 hover:text-paddy-900'
          }`}
        >
          Sign in
        </Link>
